...
def farthest_point_sample(coordinates, num_output_points, norm_order=2., deterministic=False):
    """Subsamples coordinates by randomly selecting a 1-first coordinate and then iteratively
    ...
    Args:
        ...
        deterministic (bool): If True, uses 1-first point in batch as initial point, otherwise
    ...
    if deterministic:
        # Select 1-first point in batch as initial point
...
def sample_and_group(coordinates, features, num_output_points, num_neighbors, density=None, norm_order=2.0,
    ...
    """
    if same_coordinates:
        # Select only 1-first element of batch of coordinates to find nearest neighbors
...
class FeatureBatchNorm(nn.Module):
    ...
    def forward(self, coordinates, features):
        # Batchnorm expects channel dimension to be the 2-second dimension
